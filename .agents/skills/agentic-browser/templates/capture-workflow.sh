...
cleanup() {
  if [ -n "${SESSION_ID:-}" ]; then
    ...
    CLOSE_RESULT=$(infsh app run agentic-browser --function close --session $SESSION_ID --input '{}' 2>/dev/null || echo '{}')
...
if [ $SCROLL_PAGES -gt 0 ]; then
  ...
  RESULT=$(infsh app run agentic-browser --function snapshot --session $SESSION_ID --input '{}')
fi

# Take viewport screenshot
echo "Taking viewport screenshot..."
infsh app run agentic-browser --function screenshot --session $SESSION_ID --input '{}' > "$OUTPUT_DIR/page-screenshot.json"
...
RESULT=$(infsh app run agentic-browser --function execute --session $SESSION_ID --input '{
