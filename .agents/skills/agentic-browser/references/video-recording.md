...
# Run automation
RESULT=$(infsh app run agentic-browser --function interact --session $SESSION --input '{
  "action": "click", "ref": "@e1"
}')
...
if [ "$SUCCESS" != "true" ]; then
  ...
  CLOSE_RESULT=$(infsh app run agentic-browser --function close --session $SESSION --input '{}')
...
cleanup() {
  if [ -n "$SESSION" ]; then
    infsh app run agentic-browser --function close --session $SESSION --input '{}' 2>/dev/null
  fi
}
trap cleanup EXIT
```
