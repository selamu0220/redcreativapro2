...
RESULT=$(infsh app run agentic-browser --function snapshot --session $SESSION --input '{}')
...
RESULT=$(infsh app run agentic-browser --function execute --session $SESSION --input '{
...
```bash
# Always clean up
trap 'infsh app run agentic-browser --function close --session $SESSION --input "{}" 2>/dev/null' EXIT
```
