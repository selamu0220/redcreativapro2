...
export default function EmailCollectionAdminPage() {
    ...
    try {
      ...
      if (savedQuestionnaire) {
        ...
        const defaultQuestionnaire: Questionnaire = {
          ...
          userEmail,
          title: 'Cuestionario de Contacto',
          description: 'Ayúdanos a conocerte mejor completando este breve cuestionario.',
          questions: [
            {
              id: '1',
              type: 'text',
  ...
  const generateQuestionsWithAI = async () => {
    if (!aiPrompt.trim()) {
      alert('Por favor, ingresa una descripción para generar las preguntas.');
      return;
    }
    ...
    try {
      ...
      if (questionnaire && data.questions) {
        ...
        const updatedQuestionnaire = {
          ...questionnaire,
          questions: [...questionnaire.questions, ...newQuestions],
          updatedAt: new Date().toISOString()
        };
        
        setQuestionnaire(updatedQuestionnaire);
        localStorage.setItem(`questionnaire_${userEmail}`, JSON.stringify(updatedQuestionnaire));
        setAiPrompt('');
                          ...
                          <div className="space-y-4">
                            ...
                            <div className="flex items-center">
                              <input
                                ...
                                onChange={(e) => setQuestionnaire(prev => prev ? { ...prev, isActive: e.target.checked } : null)}
                                className="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded"
                              />
                              <label htmlFor="questionnaireActive" className="ml-2 block text-sm text-gray-900 dark:text-white">
                                Cuestionario activo
                              </label>
                            </div>
                          </div>
                      ...
                      <div className="bg-white dark:bg-gray-800 shadow sm:rounded-lg">
                        <div className="px-4 py-5 sm:p-6">
                          ...
                          <div className="space-y-4">
                            <div>
                              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                                Describe qué tipo de información quieres recopilar
                              </label>
                              <MobileOptimizedTextarea
                                value={aiPrompt}
                                onChange={(e) => setAiPrompt(e.target.value)}
                                placeholder="Ej: Quiero conocer las preferencias de mis clientes sobre productos tecnológicos, su presupuesto y frecuencia de compra"
                                rows={3}
                                maxLength={500}
                              />
                            </div>
                            ...
                            >
                              {generatingQuestions ? 'Generando...' : 'Generar Preguntas con IA'}
                            </button>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="bg-white dark:bg-gray-800 shadow sm:rounded-lg">
                        <div className="px-4 py-5 sm:p-6">
                          <div className="flex justify-between items-center mb-4">
                            <h3 className="text-lg leading-6 font-medium text-gray-900 dark:text-white">
                              Preguntas del Cuestionario ({questionnaire.questions.length})
                            </h3>
                            <button
                          ...
                          <div className="space-y-4">
                                ...
                                <div className="space-y-3">
                                  ...
                                  <div className="flex items-center">
                                    <input
                                      ...
                                      className="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded"
                                    />
                                    <label htmlFor={`required-${question.id}`} className="ml-2 block text-sm text-gray-900 dark:text-white">
                            ...
                            {questionnaire.questions.length === 0 && (
                              <div className="text-center py-8 text-gray-500 dark:text-gray-400">
                                No hay preguntas en el cuestionario. Usa la IA para generar preguntas o agrega una manualmente.
                              </div>
                            )}
                          </div>
                        </div>
