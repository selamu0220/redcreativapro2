...
const EditorPanel = dynamic(() => import('./EditorPanel'), { ssr: false, loading: () => <div className="p-4 text-xs text-muted-foreground">Cargando Editor...</div> });
...
export default function AdvancedDockLayout() {
    ...
    return (
        <div className="h-full w-full flex flex-row overflow-hidden bg-background text-foreground">
            ...
            {/* Mobile/Resizer Note: This is a static grid for maximum stability to unblock production. Resizable panels can be re-introduced carefully later. */}
        </div>
    );
}
