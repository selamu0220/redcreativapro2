    ...
    const handleSendMessage = async () => {
        ...
        try {
            ...
            if (updateMatch) {
                ...
                aiResponse = aiResponse.replace(updateRegex, '').trim();
                if (!aiResponse) aiResponse = "He actualizado el documento con tus cambios.";
            }

            // SAFETY: Final cleanup to ensure no protocol tags leak to UI
            aiResponse = aiResponse.replace(/(?:\*\*?)?:::\s*UPDATE[-_ ]?DOCUMENT\s*:::(?:\*\*?)?/gi, '').trim();
