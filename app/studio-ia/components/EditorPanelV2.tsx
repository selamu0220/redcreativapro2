...
export default function EditorPanel() {
    ...
    const handleSave = React.useCallback(async () => {
        ...
        try {
            ...
            if (!response.ok) {
                // Handle Auth Error first - silent in console, loud in UI
                if (response.status === 401 || data.error === 'Unauthorized') {
                    toast.error("Tu sesión ha expirado. Por favor, inicia sesión de nuevo.", {
                        action: {
                            label: "Entendido",
                            onClick: () => console.log("User acknowledged auth error")
                        }
                    });
                    return;
                }

                console.error("Save Server Error:", data); // Only log real errors
    ...
    }, [setContent, docTitle, setDocTitle]);

    const handleExport = React.useCallback(async (format: 'pdf' | 'docx' | 'md') => {
        try {
    ...
    }, [handleSave, handleImportClick, handleExport, performPublish, content, docTitle]);
