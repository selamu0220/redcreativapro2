...
export default function MobileNavigation({ currentPath }: MobileNavigationProps) {
  ...
  return (
    <>
        ...
        <div className="flex items-center justify-between px-4 py-3">
          ...
          <div className="flex items-center space-x-2">
            ...
            <button
              ...
              className={`p-2 rounded-lg transition-all duration-300 ${isMenuOpen
      ...
      <nav className={`fixed top-0 right-0 h-full w-80 max-w-[85vw] bg-background border-l border-border z-50 transform transition-transform duration-300 ease-in-out ${isMenuOpen ? 'translate-x-0' : 'translate-x-full'
        }`}>
              ...
              <div className="space-y-1">
                {publicMenuItems.map((item) => (
                  <Link
                    key={item.href}
                    href={item.href}
                    className={`flex items-center space-x-3 px-4 py-3 rounded-lg transition-all duration-200 ${currentPath === item.href
        ...
        {bottomNavItems.map((item) => {
          ...
          return (
            <Link
              key={item.href}
              href={item.href}
              className={`flex flex-col items-center justify-center p-2 rounded-lg transition-all duration-200 min-w-[60px] ${isActive
                ? 'text-primary scale-110'
                : 'text-muted-foreground hover:text-foreground hover:scale-105'
                }`}
            >
              <span className={`text-xl mb-1 transition-transform duration-200 ${isActive ? 'animate-bounce' : ''
                }`}>
                {item.icon}
              </span>
