...
export function AuthAwareNav() {
  ...
  // Cargar estado cacheado al montar
  useEffect(() => {
    const cached = localStorage.getItem('kinde_auth_state')
    if (cached) {
  ...
  useEffect(() => {
    if (!isLoading && mounted) {
      localStorage.setItem('kinde_auth_state', String(isAuthenticated))
  ...
  // Si está cargando pero tenemos cache, usar el cache
  if (isLoading && cachedAuth !== null) {
    // Renderizar basado en cache mientras carga
    if (cachedAuth && user) {
  ...
  // Mostrar skeleton solo si está cargando y no hay cache
  if (isLoading) {
    return <LoadingSkeleton />
  }
