...
export default function ClientHomePage({ initialLang }: ClientHomePageProps) {
  const [currentLang, setCurrentLang] = useState<LanguageCode>(initialLang);
  const { t, isClient } = useSimpleTranslations();

  // Update current language when initialLang changes
  useEffect(() => {
    setCurrentLang(initialLang);
  }, [initialLang]);

  // Use a simple translation function that works with the current language
  const getTranslation = (key: string) => {
    // Fallback translations for basic keys
    const fallbackTranslations = {
      campaigns: currentLang === 'en' ? 'AI Campaigns' : 'Campañas IA',
      membership: currentLang === 'en' ? 'Membership' : 'Membresía',
      blog: 'Blog',
      login: currentLang === 'en' ? 'Sign In' : 'Iniciar Sesión',
      demo: currentLang === 'en' ? 'View Demo' : 'Ver Demo',
      mainTitle: 'Red Creativa Pro',
      subtitle: currentLang === 'en' ? 'Hispanic AI Marketing Platform' : 'Plataforma Hispana de Marketing con IA',
      description: currentLang === 'en' 
        ? 'Create content, manage campaigns and automate your marketing with artificial intelligence tools designed specifically for the Spanish-speaking market.'
      ...
      joinPlatform: currentLang === 'en' ? '🚀 Join Red Creativa Pro' : '🚀 Unirse a Red Creativa Pro'
    };
    
    // Use the hook's translation if available, otherwise use fallback
    return isClient ? t(key as any) : (fallbackTranslations[key as keyof typeof fallbackTranslations] || key);
  };
